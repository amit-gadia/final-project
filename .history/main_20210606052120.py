...
def badd():
    ...
    val=(noo,aa[0][-1],rno,issudate,return_date,aa[0][1])
    cur.execute(sql,val)
    conn.commit()
    return"Book Issue SSuccessfully"
...
def rmbok():
    ...
    bname=cur.fetchall()
    lenBname=len(bname)
    return render_template('library/rviewbook.html',datapp=datap,dataaa=dataa,bdata=bname,lenBnamep=lenBname,lendataAp=lendataA,lendataBa=lendataB)
